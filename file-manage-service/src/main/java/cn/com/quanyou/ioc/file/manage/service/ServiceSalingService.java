...
import cn.com.quanyou.ioc.file.manage.dao.IServiceSalingImportDao;
...
@Service
@Slf4j
public class ServiceSalingService extends ExcelImportService implements IExcelImportService<ServiceSalingBean> {

    @Autowired
    private IServiceSalingImportDao importDao;
    ...
    public void importExcel(AnalysisTaskBean task, ExcelModel excelModel) {
        ...
        ExcelUtilsMap<ServiceSalingBean> excelUtils = new ExcelUtilsMap<>(ServiceSalingBean.class,this);
    ...
    public List<AnalysisTaskErrorBean> relationDataCheck(List validList, List unvalidList, List entityList) {
        ...
        List<ServiceSalingBean> list = entityList;

        List<ShopBasicInfoBean> basicList = shopBasicDao.queryAllFormal();

        for(ServiceSalingBean entity : list){
    ...
    public void dataValidate(String taskId,String loginUser) {
        ...
        // 20190925 新增过滤出与选择的店铺名称相同的数据来进行校验，忽略其他店铺的数据
        ...
        BeanUtils<ServiceSalingBean> beanUtils = new BeanUtils<>(ServiceSalingBean.class);
    ...
    public ResultInfo<String,String> copyDataFromTemp(String taskId) {
        ...
        int count = importDao.queryMistakeAndUncheckedCountByShopName(taskId, shopName);
        if(count>0){
    ...
    @Override
    public ResultInfo<PageInfo<ServiceSalingBean>, String> search(SearchParamVo searchParamVo, PageInfo<ServiceSalingBean> pageInfo) {
        List<ServiceSalingBean> dataList = importDao.search(searchParamVo,pageInfo.getStartRow(),pageInfo.getEndRow());
        ...
        ResultInfo<PageInfo<ServiceSalingBean>, String> resultInfo = new ResultInfo<>();
