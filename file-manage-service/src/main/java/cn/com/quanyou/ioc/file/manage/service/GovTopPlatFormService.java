...
import cn.com.quanyou.ioc.file.manage.dao.IGovTopPlatformTiDao;
...
import cn.com.quanyou.ioc.file.manage.vo.ExcelImportEntities.GovTopPlatformTi;
...
/**
 * @Description: 各平台各渠道访客数量top5
 * @Author: yangli
 * @Date: 2019/9/11 - 14:32
**/
@Service
@Slf4j
public class GovTopPlatFormService extends ExcelImportService implements IExcelImportService<GovTopPlatformTi> {

    @Autowired
    private IGovTopPlatformTiDao importDao;
    ...
    public void importExcel(AnalysisTaskBean task, ExcelModel excelModel) {
        ...
        ExcelUtilsMap<GovTopPlatformTi> excelUtils = new ExcelUtilsMap<>(GovTopPlatformTi.class,this);
    ...
    public void dataValidate(String taskId,String loginUser) {
        ...
        BeanUtils<GovTopPlatformTi> beanUtils = new BeanUtils<>(GovTopPlatformTi.class);
    ...
    public List<AnalysisTaskErrorBean> relationDataCheck(List validList, List unvalidList, List entityList) {
        ...
        List<GovTopPlatformTi> list = entityList;

       /* List<Map<String,String>> shopNames = dataValidateService.queryShopNames();

        for(GovTopPlatformTi entity : list){
    ...
    @Override
    public ResultInfo<PageInfo<GovTopPlatformTi>, String> search(SearchParamVo searchParamVo, PageInfo<GovTopPlatformTi> pageInfo) {
        return null;
    }
}
